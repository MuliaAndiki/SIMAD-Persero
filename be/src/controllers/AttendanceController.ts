...
class AttendanceController {
  ...
  public async getById(c: AppContext) {
    try {
      ...
      const userId = c.user!.roles?.some((r) => r.toLowerCase() === 'intern')
        ? c.user!.id
        : undefined;
  ...
  public async getSupervisorDashboard(c: AppContext) {
    try {
      const query = c.query as { date?: string } | undefined;
      const data = await attendanceService.getSupervisorDashboard(c.user!.id, query?.date);
      return HttpResponse(c).ok(data);
    } catch (error) {
      return this.handleError(c, error);
    }
  ...
  public async getHistory(c: AppContext) {
    try {
      ...
      const { data, meta } = await attendanceService.getHistory(query, c.user);
      return HttpResponse(c).ok(data, meta);
    } catch (error) {
      return this.handleError(c, error);
    }
