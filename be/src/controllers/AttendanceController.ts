...
class AttendanceController {
  ...
  public async getMyAttendance(c: AppContext) {
    try {
      ...
      const data = await attendanceService.getMyAttendance(c.user!.id, query);
  ...
  public async getById(c: AppContext) {
    try {
      const data = await attendanceService.getById(c.params.attendanceId);
      return HttpResponse(c).ok(data);
    } catch (error) {
      return this.handleError(c, error);
    }
  ...
  public async getHistory(c: AppContext) {
    try {
      ...
      const data = await attendanceService.getHistory(query);
      return HttpResponse(c).ok(data);
    } catch (error) {
      return this.handleError(c, error);
    }
  ...
  public async exportAttendance(c: AppContext) {
    try {
      ...
      const data = await attendanceService.exportAttendance(query);
      return HttpResponse(c).ok(data, undefined, 'Export berhasil.');
    } catch (error) {
      return this.handleError(c, error);
    }
  }
