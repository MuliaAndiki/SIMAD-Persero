...
class SupervisorController {
  ...
  public async updateAccount(c: AppContext) {
    try {
      ...
      const data = await supervisorService.updateAccount(c.user!.id, c.params.supervisorId, body);
      return HttpResponse(c).ok(data, undefined, 'Akun supervisor berhasil diperbarui.');
    } catch (error) {
      return this.handleError(c, error);
    }
  }
  ...
  public async deleteAccount(c: AppContext) {
    try {
      ...
      return HttpResponse(c).ok(null, undefined, 'Akun supervisor berhasil dihapus.');
    } catch (error) {
      return this.handleError(c, error);
    }
  }
