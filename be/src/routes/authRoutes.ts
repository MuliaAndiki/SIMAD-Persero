...
class AuthRouter {
  ...
  private routes() {
    ...
    this.authRouter.post(
      ...
      (c: AppContext) => AuthController.sendVerifyEmail(c),
      {
        ...
        detail: {
          ...
          description:
            "Mengirim email berisi token verifikasi ke email pengguna.",
          tags: ["Auth"],
        },
      },
    );
    ...
    this.authRouter.post(
      "/verify-email",
      (c: AppContext) => AuthController.verifyEmail(c),
      {
        body: VerifyEmailDto,
        detail: {
    ...
    this.authRouter.post(
      "/oauth",
      (c: AppContext) => AuthController.googleLogin(c),
      {
        body: GoogleLoginDto,
        beforeHandle: [rateLimit(RateLimitRule.LOGIN).beforeHandle],
        detail: {
    ...
    this.authRouter.post(
      "/refresh-token",
      (c: AppContext) => AuthController.refreshToken(c),
      {
    ...
    this.authRouter.post(
      ...
      (c: AppContext) => AuthController.logout(c),
      {
        ...
        detail: {
          ...
          description:
            "Membatalkan sesi saat ini. Membutuhkan header Authorization Bearer.",
          tags: ["Auth"],
        },
      },
    );
    ...
    this.authRouter.post(
      "/logout-all",
      (c: AppContext) => AuthController.logoutAll(c),
      {
        beforeHandle: [verifyToken().beforeHandle],
        detail: {
    ...
    this.authRouter.patch(
      "/change-password",
      (c: AppContext) => AuthController.changePassword(c),
      {
    ...
    this.authRouter.patch(
      "/change-email",
      (c: AppContext) => AuthController.changeEmail(c),
      {
        body: ChangeEmailDto,
        beforeHandle: [verifyToken().beforeHandle],
        detail: {
          summary: "Ubah email",
          description:
            "Mengajukan perubahan email dan mengirim token konfirmasi.",
          tags: ["Auth"],
        },
      },
    );
    ...
    this.authRouter.post(
      ...
      (c: AppContext) => AuthController.changeEmailVerify(c),
      {
        ...
        detail: {
          ...
          description:
            "Menyelesaikan perubahan email menggunakan token konfirmasi.",
          tags: ["Auth"],
        },
      },
    );
    ...
    this.authRouter.get(
      "/sessions",
      (c: AppContext) => AuthController.sessions(c),
      {
        beforeHandle: [verifyToken().beforeHandle],
        detail: {
