...
class AttendanceRouter {
  ...
  private routes() {
    ...
    this.attendanceRouter.post(
      "/check-in",
      (c: AppContext) => attendanceController.checkIn(c),
      {
        beforeHandle: [
          verifyToken().beforeHandle,
          requireRole(["INTERN"]).beforeHandle,
          rateLimit({ ...RateLimitRule.ATTENDANCE, keyGenerator: keyByUser })
            .beforeHandle,
    ...
    this.attendanceRouter.post(
      "/check-out",
      (c: AppContext) => attendanceController.checkOut(c),
      {
        beforeHandle: [
          verifyToken().beforeHandle,
          requireRole(["INTERN"]).beforeHandle,
    ...
    this.attendanceRouter.get(
      "/me",
      (c: AppContext) => attendanceController.getMyAttendance(c),
      {
        beforeHandle: [
          verifyToken().beforeHandle,
          requireRole(["INTERN"]).beforeHandle,
    ...
    this.attendanceRouter.get(
      "/today",
      (c: AppContext) => attendanceController.getToday(c),
      {
        beforeHandle: [
          verifyToken().beforeHandle,
          requireRole(["INTERN"]).beforeHandle,
    ...
    this.attendanceRouter.get(
      "/summary",
      (c: AppContext) => attendanceController.getSummary(c),
      {
        beforeHandle: [
          verifyToken().beforeHandle,
          requireRole(["INTERN"]).beforeHandle,
    ...
    this.attendanceRouter.get(
      "/history",
      (c: AppContext) => attendanceController.getHistory(c),
      {
        beforeHandle: [
          verifyToken().beforeHandle,
    ...
    // 16.10 GET /attendance/export (HR_ADMIN, SUPERVISOR, INTERN)
    this.attendanceRouter.get(
      "/export",
    ...
    // 16.4 GET /attendance/:id (HR_ADMIN, SUPERVISOR, INTERN)
    this.attendanceRouter.get(
      "/:attendanceId",
