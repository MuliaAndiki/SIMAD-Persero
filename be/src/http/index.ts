...
export function HttpResponse(c: AppContext) {
  return {
    ...
    noContent: (message = "Tidak ada konten") =>
      c.json?.({ status: 204, message }, 204),
    ...
    unprocessable: (
      message = "Entitas tidak dapat diproses",
      code?: ErrorCode,
    ...
    badGateway: (message = "Gateway bermasalah") =>
      c.json?.({ status: 502, message }, 502),
...
export function handleAppError(c: AppContext, error: unknown) {
  if (error instanceof AppError) {
    ...
    if (status >= 500) {
      console.error(`[AppError 💥] ${status} - ${message}`, error);
    } else {
      console.warn(
        `[AppWarn ⚠️] ${status} - ${message} (Code: ${code || "N/A"})`,
      );
    }
