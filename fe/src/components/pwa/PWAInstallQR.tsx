  ...
  return (
    ...
    >
      ...
      <motion.div layout className="relative p-2 bg-white rounded-xl">
        ...
        />
        <motion.div layout className="absolute inset-0 border-2 border-[#00A2E9]/20 rounded-xl pointer-events-none" />
      </motion.div>
      <motion.div layout className="flex items-center gap-1.5 text-xs text-muted-foreground text-center">
        <ScanLine className="size-3.5 shrink-0" />
        <span>Scan QR Code ini dengan HP<br/>untuk install SIMAD </span>
      </motion.div>
    </motion.div>
  );
}
