...
export const REVIEWABLE_STATUSES: ApplicationStatusValue[] = [
  "SUBMITTED",
  "UNDER_REVIEW",
];
...
export function ApplicationReviewDetail({
  ...
  const reviewable = REVIEWABLE_STATUSES.includes(
      ...
      <div className="flex flex-col gap-2 rounded-lg border p-4">
        ...
        <div className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
          <ApplicationDetailField
            label="Nama"
            value={app.internProfile?.user.fullName}
          />
          <ApplicationDetailField
            label="Email"
            value={app.internProfile?.user.email}
          />
          <ApplicationDetailField
            label="NIM"
            value={app.internProfile?.studentNumber}
          />
          <ApplicationDetailField
            label="Institusi"
            value={app.internProfile?.institution?.name}
          />
          <ApplicationDetailField
            label="Jurusan"
            value={app.internProfile?.major?.name}
          />
          <ApplicationDetailField
            label="No. HP"
            value={app.internProfile?.phone}
          />
        </div>
      </div>

      {app.internProfile?.profileSkills &&
        app.internProfile.profileSkills.length > 0 && (
          <div className="flex flex-col gap-2 rounded-lg border p-4">
            <span className="text-xs font-medium text-muted-foreground">
              Keterampilan
            </span>
      ...
      {app.introductionLetterFile && (
        <div className="flex items-center gap-3 rounded-lg border p-3">
          ...
          <Button asChild variant="outline" size="sm">
            <a
              href={app.introductionLetterFile.url}
              target="_blank"
              rel="noreferrer"
            >
              Lihat
