...
function walk(dir) {
  ...
  list.forEach((file) => {
    ...
    if (stat && stat.isDirectory()) {
      ...
      if (file.endsWith('mutate.ts')) {
        // we only want mutate.ts files
        results.push(file);
      }
    }
  });
...
files.forEach((file) => {
  ...
  if (!content.includes(contextType)) {
    content =
      `import { ${snapshotFn}, type ${contextType} } from '${cacheImportPath}';\n` + content;
    changed = true;
  }
  ...
  const fnRegex =
    /export\s+function\s+([a-zA-Z0-9_]+)\s*\(\)\s*\{([\s\S]*?)return\s+useMutation(?:<[^>]*>)?\(\{(.*?)\}\);/gm;
  ...
  content = content.replace(fnRegex, (match, hookName, bodyBeforeMutate, mutateConfig) => {
    ...
    if (newBody.includes('useQueryClient()')) {
      newBody = newBody.replace(
        /const\s+queryClient\s*=\s*useQueryClient\(\);?/g,
